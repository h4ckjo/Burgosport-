<?php
	...
	if($m=="B"){
		...
		if(mysqli_query($conn,$sql)){
			header("location:ciclismoABC.php");
		}
	...
	if (isset($_POST['nombre'])){
		...
		if($nombre==""){
			...
			if ($id==""){
				$sql = "INSERT INTO productos VALUES(0,'2','".$subtipo."', ";
				$sql .= "'".$nombre."', ";
				$sql .= "'".$descripcion."', ";
				$sql .= "'".$tallas."', ";
				$sql .= $precio.", ";
...
//leemos la tabla de productos
	...
	//subtipo1 
	if($m=="S"){
		$sql="SELECT * FROM productos WHERE tipo='2' ORDER BY nombre AND subtipo";
		$r=mysqli_query($conn, $sql);
	...
	if($m=="A" || $m=="C"){
		$sql="SELECT id,nombre FROM productos WHERE tipo='2' ORDER BY subtipo";
		$r=mysqli_query($conn, $sql);
		//creamos un arreglo vacio
		$productos=array();
		while($row=mysqli_fetch_assoc($r)){
...
<script>
		...
		document.getElementById("borrar").onclick = function() {
			if(confirm("¿Estás seguro de eliminar el registro?\nUna vez eliminado el registro no podrá recuperarse")){
				var id = <?php print $id; ?>;
				window.open("ciclismoABC.php?m=B&id="+id, "_self");
			}
		}
		<?php } ?>
		<?php if($m=="S"){ ?>
			document.getElementById("alta").onclick = function() {
			window.open("ciclismoABC.php?m=A", "_self");
		}
		<?php } else { ?>
			document.getElementById("volver").onclick = function() {
			window.open("ciclismoABC.php", "_self");
		}
		<?php } ?>
	}
</script>
	...
	<div class="row content">
		<div class="col-sm-3 sidenav">
			...
			<div class="well" style="margin-top:20px">
				...
				<input type="button" name="alta" value="Alta de Producto de ciclismo" class="btn btn-info" role="button" id="alta"/>
			</div>
			<div class="well" style="margin-top:20px">
				<p>Subtipo 1 Maillots</p>
				<p>Subtipo 2 Botas</p>
				<p>Subtipo 3 Bicicletas</p>
		...
		<div class="col-sm-9">
			<div class="well" style="margin-top:20px">
				<h2>ALTAS, BAJAS Y CAMBIOS DE CICLISMO</h2>
					<?php 
					if($m=="A" || $m=="C"){
			...
			<div class="well" style="margin-top:20px">
				...
				<form action="ciclismoABC.php" method="post" enctype="multipart/form-data">
				...
				if($m=="S"){
					...
					for($i=0; $i < count($productos); $i++){
						...
						print '<p><a href="ciclismoABC.php?m=C&id='.$productos[$i]["id"].'">'.$productos[$i]["nombre"].'</a></p>';
